...
def get_axis_values(column_id, lines, csv_header=['tp','fp','tn','fn']):
    ...
    *csv_header* should be the header line from the csv file passed in to the
    ...
    if type(lines[0]) != 'dict':
        lines = [{k:float(v) for k,v in zip(csv_header,line)} for line in lines]
...
#        path = os.path.join(args.create_figures, fname)
    ...
    plot_groups = [[fname for fname in files if fname[args.figure_grouping] == key] \
                   for key in plots]
    ...
    for plot_group in plot_groups:
        ...
        for line_group in line_groups:
            ...
            fh.close()
            values = np.ndarray(shape=(len(line_group),2,num_lines), dtype=np.float64) 
            
            for i,item in enumerate(line_group):
                # todo: average over the line group
                with open(item[0], 'r') as in_fh:
                    reader = csv.reader(in_fh)
                    csv_header = [field.strip().lower() for field in reader.next()]
                    
                    lines = [line for line in reader]
                    
                    x_values = get_axis_values(args.x_values, lines, csv_header)
