...
def do_test(func, in_fname, out_fname):
    with open(in_fname, 'r') as in_fh, open(out_fname, 'w') as out_fh:
        features = pickle.load(
            open('/usr/local/scratch/TEMP/features.pickle', 'r'))
...
#    strip_features_parser(in_fh, out_fh,features)
    ...
    print 'RegExp', timeit.timeit(
        stmt='do_test("strip_features_re", "/usr/local/scratch/TEMP/train/kazakhmys.train", "/usr/local/scratch/TEMP/re.train.txt")',
        setup='from __main__ import do_test', number=100) / 100.0
    print 'Parser', timeit.timeit(
        stmt='do_test("strip_features_parser", "/usr/local/scratch/TEMP/train/kazakhmys.train", "/usr/local/scratch/TEMP/parser.train.txt")',
        setup='from __main__ import do_test', number=100) / 100.0
