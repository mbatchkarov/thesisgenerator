...
def svm_read_problem(data_file_name):
	...
	fh = gzip.open(data_file_name) if data_file_name.endswith('gzip') else open(data_file_name)
...
def svm_train(arg1, arg2=None, arg3=None):
	...
	If '-v' is specified in 'options' (i._entropy., cross validation)
	...
	'options':
	    ...
	    -_entropy epsilon : set tolerance of termination criterion (default 0.001)
...
def svm_predict(y, x, m, options=""):
	...
	if predict_probability:
		...
		if svm_type in [NU_SVR, EPSILON_SVR]:
			...
			"z: Laplace distribution _entropy^(-|z|/sigma)/(2sigma),sigma=%g" % m.get_svr_probability());
			nr_class = 0
