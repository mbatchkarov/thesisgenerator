...
def _output_paths(source_file, output_dir, metric=None, fc=None,
                  classifier=None):
    ...
    cls_fn = os.path.join(output_dir, 'classifications',
        '%s.predict.txt' % cls_fn)

    return model_fn, predict_fn, cls_fn


def svm_predict(source_file, output_dir, metric=None, fc=None,
                classifier=None):
    ...
    print 'Predict \'%s\' - %s' % (
    classifier, time.strftime('%Y-%b-%d %H:%M:%S'))
    ...
    def _run_prediction(classifier, predict_y, precit_x, model, argv, model_fn,
                        q):
    ...
    while q.empty():
        ...
        p = mp.Process(target=_run_prediction,
            args=(classifier, predict_y, predict_x, argv, model_fn, q))
        p.start()
        p.join()
...
def mallet_predict(source_file, output_dir, metric=None, fc=None,
                   classifier=None):
    ...
    print 'Predict \'%s\' - %s' % (
    mallet_trainer, time.strftime('%Y-%b-%d %H:%M:%S'))
    print '--> predict data file: \'%s\'' % predict_fn
    print '--> load model from: \'%s\'' % model_fn
    print '--> write classifications to: \'%s\'' % cls_fn
    print '----> process arguments: \'%s\'' % args.prc_args
    ...
    fh = open(cls_fn, 'r')
    f_id, fpath = tempfile.mkstemp(prefix='.', suffix='.mallet',
        dir=args.output)
    ...
    gzip.open(predict_fn, 'r') as true_labels:
        for predict, correct in zip(predictions, true_labels):
            ...
            predict_cls = 1 if float(float(cols[1])) > float(
                float(cols[3])) else -1
