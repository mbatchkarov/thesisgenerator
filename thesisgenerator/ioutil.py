...
def train_fn_from_source(source_fn, output_dir, num_seen=None, fs=None,
                         fc=None, stratified=False, job_id=1):
...
def predict_fn_from_source(source_fn, output_dir, num_seen=None, fs=None,
                           fc=None, stratified=False, job_id=1):
    source_fn, _, _ = source_fn.partition('.')
    _, source_fn = os.path.split(source_fn)
...
def model_fn_from_source(source_fn, output_dir, num_seen=None, fs=None,
                         fc=None, classifier=None, stratified=False, job_id=1):
    source_fn, _, _ = source_fn.partition('.')
    _, source_fn = os.path.split(source_fn)
...
def find_mallet():
    ...
    if mallet_exec is None:
        raise RuntimeError(
            'Can not find mallet executable. Please specify the '\
