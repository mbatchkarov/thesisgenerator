...
def sort(features_dict, metric_name):
    ...
    sorted_features = sorted(features_dict.keys(),
                             key=lambda k: metric(features_dict[k]),
                             reverse=True)
...
def bns(feature, *args):
    ...
    try:
        ...
        if tpr == 0:
            invcdf_tpr = _min_float # -inf
        elif tpr == 1:
            invcdf_tpr = _max_float # inf

    try:
        invcdf_fpr = _ltqnorm(fpr)
    except ValueError:
        if fpr == 0:
            invcdf_fpr = _min_float # -inf
        elif fpr == 1:
            invcdf_fpr = _max_float # inf
...
def entropy(*args):
    ...
    # component to 0
    e = map(lambda arg: 0 if arg == 0 else arg / total * np.log2(arg / total),
            args)
    return -sum(e)
...
def _ltqnorm(p):
    ...
    if p <= 0 or p >= 1:
        ...
        raise ValueError(
            "Argument to _ltqnorm %f must be in open interval (0,1)" % p)
    ...
    if phigh < p:
        ...
        return -(
            ((((c[0] * q + c[1]) * q + c[2]) * q + c[3]) * q + c[4]) * q + c[
                5]) / \
               ((((d[0] * q + d[1]) * q + d[2]) * q + d[3]) * q + 1)
